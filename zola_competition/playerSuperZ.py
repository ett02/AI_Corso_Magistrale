...
_SHARED_HISTORY = [[[[0] * MAX_BOARD_SIZE for _ in range(MAX_BOARD_SIZE)]
                    ...
                    for _ in range(MAX_BOARD_SIZE)]
_SHARED_AGE = [0]  # lista mutabile per condivisione tra istanze
...
class ZolaAI:
    ...
    # ── Euristica (NegaMax: dal punto di vista di state.to_move) ─────

    def evaluate_state(self, state, depth_from_root):
        """
        Valutazione euristica in stile NegaMax.
        Restituisce il punteggio dal punto di vista del giocatore di turno.
        Lazy eval: ai nodi profondi (depth_from_root >= 4) usa solo il materiale.
        """
        ...
        material = (my_count - opp_count) * 100

        # LAZY EVAL: ai nodi profondi, solo materiale (molto più veloce)
        if depth_from_root >= 4:
            return material

        # FULL EVAL: materiale + catture + mobilità con pesi phase-aware
        ...
        # Pesi dinamici in base alla fase di gioco
        if total_pieces <= 12:
            # ENDGAME: mobilità cruciale, catture ancora importanti
            capture_weight = 8
            mobility_weight = 3
        elif total_pieces <= 40:
            # MEDIOGIOCO: bilanciamento standard
            capture_weight = 10
            mobility_weight = 1
        else:
            # APERTURA: catture aggressive, mobilità irrilevante (board piena)
            capture_weight = 12
            mobility_weight = 0

        captures = (my_captures - opp_captures) * capture_weight
        mobility = (len(my_moves) - len(opp_moves)) * mobility_weight

        return material + captures + mobility
    ...
    def update_killers(self, move, depth):
        """Salva una mossa quiet che ha causato un beta-cutoff come killer."""
    ...
    def update_history(self, move, depth):
        """Incrementa lo score nella history table per una mossa che taglia."""
        if move != "PASS" and not move[2]:
            (fr, fc), (tr, tc), _ = move
    ...
    def negamax(self, state, depth, alpha, beta, current_hash, depth_from_root):
        """
        NegaMax con:
        ...
        if depth == 0 or self.game.is_terminal(state) or not legal_moves:
            return self.evaluate_state(state, depth_from_root), None

        # ── Futility Pruning (depth <= 2, solo mosse quiet) ──
        futility_prune = False
        if depth <= 2:
            static_eval = self.evaluate_state(state, depth_from_root)
            margin = 150 if depth == 1 else 350
            if static_eval + margin <= alpha:
                # Posizione troppo pessima: cerca solo catture
        ...
        for i, move in enumerate(legal_moves):
            ...
            dfr = depth_from_root + 1

            # ── LMR: Late Move Reduction ──
            # Dopo le prime 3 mosse, se la mossa è quiet e depth >= 3,
            # cerca a profondità ridotta. Se sorprende, ri-cerca full.
            ...
            if do_lmr:
                ...
                value = -self.negamax(
                    child_state, depth - 2, -(alpha + 1), -alpha,
                    child_hash, dfr
                )[0]
                ...
                if value > alpha:
                    ...
                    value = -self.negamax(
                        child_state, depth - 1, -beta, -alpha,
                        child_hash, dfr
                    )[0]
            else:
            ...
            if alpha >= beta:
                # Beta cutoff: aggiorna killers e history
    ...
    def search(self, state):
        ...
        # Decadimento della history table (dimezza i valori)
        for r1 in range(state.size):
            for c1 in range(state.size):
                for r2 in range(state.size):
                    for c2 in range(state.size):
                        self.history[r1][c1][r2][c2] >>= 1  # //= 2

        # Pulizia TT: rimuovi entries vecchie se troppo grande
        ...
        try:
            while True:
                # Finestra di aspirazione (solo da depth > 1)
                if depth > 1:
                    a = prev_score - WINDOW
                    b = prev_score + WINDOW
                else:
        ...
        print(f"[AI SuperZ {self.root_player}] Profondità raggiunta: {self.completed_depth}. Mossa scelta: {best_move}")
        return best_move
